...
from vocalremover.lib import dataset
from vocalremover.lib import nets
from vocalremover.lib import spec_utils
from vocalremover.lib import utils
...
def run_silent(wav_path, model_path, sr=44100, hop_length=1024, n_fft=2048, batchsize=4, cropsize=256):
    ...
    X, sr = librosa.load(wav_path, sr, False, dtype=np.float32, res_type='kaiser_fast')
    ori_name = os.path.splitext(wav_path)[0]
    ...
    sf.write('{}_Instruments.wav'.format(ori_name), wave.T, sr)

    wave = spec_utils.spectrogram_to_wave(v_spec, hop_length=hop_length)
    sf.write('{}_Vocals.wav'.format(ori_name), wave.T, sr)
    
    return ori_name + '_Instruments.wav', ori_name + '_Vocals.wav'
